...
<div class="container" id="containerInicial">
		...
		<input type="hidden" name="codigo_livro" value="${livro.codigo_livro}" >
			...
			<select id="categoria_livro" class="form-control" name="categoria_livro" required>
    			<option value="${livro.categoria_livro}">${livro.categoria_livro}</option>
    			<c:forEach var="categoria" items="${categorias}" > 
    			    <c:set var = "Categoriaaux1" value = "${livro.categoria_livro}"/>
    				<c:set var = "Categoriaaux2" value = "${categoria.nome}"/>
    				<c:if test="${Categoriaaux1 != Categoriaaux2}"> 
